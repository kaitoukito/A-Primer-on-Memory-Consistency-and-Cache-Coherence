...
4.1 TSO/X86 的动机
处理器核心长期以来一直使用 write buffer (store buffer) 来保存已提交（退休）的存储，直到内存系统的其余部分可以处理这些存储。当存储提交时，存储进入写入缓冲区，当要写入的块处于读写 coherence 状态的高速缓存中时，存储退出写入缓冲区。值得注意的是，一个存储可以在缓存获得写入块的读写 coherence 权限之前进入写入缓冲区；因此，写缓冲区隐藏了为存储未命中提供服务的延迟。因为存储很常见，所以能够避免在大多数存储中停滞不前是一个重要的好处。此外，不停止核心似乎是明智的，因为核心不需要任何东西，因为存储试图更新内存而不是核心状态。
...
在构建多核处理器时，使用多个核心似乎很自然，每个核心都有自己的旁路写入缓冲区，并假设写入缓冲区在架构上仍然是不可见的。
...
核心 C1 执行存储 S1，但在其写缓冲区中缓冲新存储的 NEW 值。
同样，核心 C2 执行存储 S2 并将新存储的 NEW 值保存在其写缓冲区中。
接下来，两个核心执行各自的加载 L1 和 L2，并获得旧值 0。
最后，两个核心的写缓冲区使用新存储的值 NEW 更新内存。
...
SPARC 和后来的 x86 选择的选项是放弃 SC，转而支持内存一致性模型，该模型允许在每个核心上直接使用先进先出 (FIFO) 写入缓冲区。新模型 TSO 允许结果“(r1, r2) = (0, 0)”。这个模型让一些人感到惊讶，但事实证明，对于大多数编程习惯来说，它的行为就像 SC 一样，并且在所有情况下都得到了很好的定义。
...
3. 第 (1) 部分必须扩充以定义 FENCE：/*Change3: FENCEs Order Everything*/
...
人们普遍认为 x86 内存模型等效于 TSO（用于普通可缓存内存和普通指令），但据我们所知，AMD 和 Intel 都没有对此做出保证，也没有发布正式的 x86 内存模型规范。 AMD 和英特尔通过示例和散文公开定义 x86 内存模型，这一过程在 Sewell 等人的第 2 节中得到了很好的总结 [15]。所有的例子都符合 TSO，所有的散文似乎都符合 TSO。只有当 x86 内存模型的公开、正式的描述可用时，才能证明这种等价性。如果反例显示了 TSO 不允许的 x86 执行、x86 不允许的 TSO 执行或两者兼有，则这种等价性可能会被推翻。
