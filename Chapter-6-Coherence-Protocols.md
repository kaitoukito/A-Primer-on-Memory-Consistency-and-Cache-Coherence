# 第六章：Coherence 协议
在本章中，我们回到了我们在第 2 章中介绍的 cache coherence 主题。我们在第 2 章中定义了 coherence，以便理解 coherence 在支持 consistency 方面的作用，但我们没有深入研究特定的 coherence 协议是如何工作的，或者它们是如何实现的。 本章一般性地讨论 coherence 协议，然后我们将在接下来的两章中讨论特定的协议类。 我们从 6.1 节开始介绍 coherence 协议如何工作的整体情况，然后在 6.2 节展示如何指定协议。 我们在第 6.3 节中介绍了一个简单而具体的 coherence 协议示例，并在第 6.4 节中探讨了协议设计空间。

## 6.1 整体情况
Coherence 协议的目标是通过强制执行第 2.3 节中介绍并在此重申的不变量来保持 coherence。

单写多读 (SWMR) 不变量。对于任何内存位置 A，在任何给定（逻辑）时间，仅存在一个可以写入 A（也可以读取它）的核心或可能只读取 A 的一些核心。
数据值 (Data-Value) 不变量。一个时间段 (epoch) 开始时的内存位置值与其最后一个读写时间段结束时的内存位置值相同。
为了实现这些不变量，我们将每个存储结构（每个缓存和 LLC/内存）关联到一个称为一致性控制器 (coherence controller) 的有限状态机。这些一致性控制器的集合构成了一个分布式系统，其中控制器相互交换消息，以确保对于每个块，SWMR 和数据值不变量始终保持不变。 这些有限状态机之间的交互由一致性协议指定。

一致性控制器有几个职责。缓存中的一致性控制器，我们称之为缓存控制器 (cache controller)，如图 6.1 所示。缓存控制器必须为来自两个来源的请求提供服务。在“核心端”，缓存控制器与处理器核心连接。控制器接受来自核心的 load 和 store，并将 load 值返回给核心。缓存未命中导致控制器通过为包含核心访问的位置的块发出一致性请求（例如，请求只读权限）来启动一致性事务。这一一致性请求通过互连网络发送到一个或多个一致性控制器。事务由请求和为满足请求而交换的其他消息（例如，从另一个一致性控制器发送到请求者的数据响应消息）组成。作为每个事务的一部分发送的事务类型和消息取决于特定的一致性协议。
...
在缓存控制器的“网络侧”，缓存控制器通过互连网络与系统的其余部分连接。 控制器接收它必须处理的一致性请求和一致性响应。 与核心端一样，传入的一致性消息的处理取决于特定的一致性协议。

LLC/内存处的一致性控制器，我们称之为内存控制器 (memory controller)，如图 6.2 所示。 内存控制器类似于缓存控制器，只是它通常只有一个网络端。 因此，它不会发出一致性请求（代表加载或存储）或接收一致性响应。 其他代理（例如 I/O 设备）的行为可能类似于缓存控制器、内存控制器或两者都是，取决于它们的特定要求。
...
每个一致性控制器实现一组有限状态机——逻辑上一个独立但每个块相同的有限状态机——并根据块的状态接收和处理事件（例如，传入的一致性消息）。 对于类型 E 的事件（例如，从核心到缓存控制器的存储请求）到块 B，一致性控制器采取的动作是 E 和 B 状态（例如，只读）的函数。采取这些动作后，控制器可能会改变 B 的状态。
