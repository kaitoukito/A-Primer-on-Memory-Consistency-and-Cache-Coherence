...
## 5.4 无数据竞争程序的连续一致性
孩子们和计算机架构师都希望“拥有他们的蛋糕并吃掉它”。对于内存一致性模型，这可能意味着程序员能够使用（相对）直观的 SC 模型进行推理，同时仍能获得在 XC 等宽松模型上执行的性能优势。

幸运的是，对于重要的无数据竞争 (data-race-free, DRF) 程序 [3]，可以同时实现这两个目标。通俗地说，当两个线程访问同一个内存位置时，会发生数据竞争，其中至少一个访问是写入，并且没有干预同步操作。数据竞争通常（但不总是）是编程错误的结果，许多程序员试图编写 DRF 程序。用于 DRF 编程的 SC (SC for DRF programming) 要求程序员通过编写正确同步的程序并标记同步操作来确保程序是 SC 下的 DRF；然后它要求实现者通过将标记的同步操作映射到宽松内存模型支持的 FENCE 和 RMW 来确保宽松模型上 DRF 程序的所有执行也是 SC 执行。XC 和大多数商业宽松内存模型都有必要的 FENCE 指令和 RMW 来恢复 SC。此外，这种方法还可以作为 Java 和 C++ 高级语言 (HLL) 内存模型的基础（第 5.9 节）。

让我们用两个例子来激励 "SC for DRF"。表 5.7 和表 5.8 都描述了核心 C1 存储两个位置（S1 和 S2）而核心 C2 以相反顺序加载这两个位置（L1 和 L2）的示例。示例不同，因为 Core C2 在表 5.7 中没有同步，但在表 5.8 中获取与 Core C1 相同的锁。
![image](https://github.com/kaitoukito/A-Primer-on-Memory-Consistency-and-Cache-Coherence/assets/34410167/fb2f71d5-b243-4155-a789-b8cde3868fa0)
![image](https://github.com/kaitoukito/A-Primer-on-Memory-Consistency-and-Cache-Coherence/assets/34410167/e36cb650-fd45-402f-b4b7-41679b42ea09)
由于 Core C2 在表 5.7 中没有同步，它的加载可以与 Core C1 的存储同时执行。由于 XC 允许 Core C1 重新排序存储 S1 和 S2（或不），并且 Core C2 重新排序加载 L1 和 L2（或不），因此可能有四种结果，其中 (r1, r2) = (0, 0)、(0, NEW)、(NEW, 0) 或 (NEW, NEW)。例如，如果加载和存储按顺序 S2、L1、L2 再 S1 或顺序 L2、S1、S2 然后 L1 执行，则会出现输出 (0, NEW)。但是，此示例包含两个数据争用（S1 与 L2 和 S2 与 L1），因为 Core C2 未获取 Core C1 使用的锁。

表 5.8 描述了 Core C2 获取的锁与 Core C1 获取的锁相同的情况。在这种情况下，Core C1 的临界区将在 Core C2 之前完全执行，反之亦然。这允许两种结果：(r1, r2) = (0, 0) 或 (NEW, NEW)。重要的是，这些结果不受在它们各自的临界区内，Core C1 重新排序存储 S1 和 S2 和/或 Core C2 重新排序加载 L1 和 L2 的影响。“一棵树倒在树林里（重新排序的存储），但没有人听到它（没有并发加载）。”此外，XC 结果与 SC 允许的结果相同。"SC for DRF" 从这两个例子中概括为：

要么执行具有暴露 XC 的加载或存储重新排序的数据竞争，要么
XC 执行是无数据竞争的，并且与 SC 执行没有区别。
对 "SC for DRF" 的更具体理解需要一些定义。

一些内存操作被标记为同步 ("synchronization operations")，而其余的则默认标记为数据 ("data operations")。同步操作包括锁的获取和释放。
如果两个数据操作 Di 和 Dj 来自不同的核心（线程）（即不按程序顺序排序），访问相同的内存位置，并且至少有一个是存储，则它们会发生冲突 (conflict)。
如果两个同步操作 Si 和 Sj 来自不同的核心（线程），访问相同的内存位置（例如，相同的锁），并且至少一个同步操作是写入（例如，获取和释放一个自旋锁会冲突，而两者都读锁的值则不会冲突）。
如果 Si 和 Sj 冲突或者如果 Si 与某些同步操作 Sk 冲突，则两个同步操作 Si 和 Sj 会传递冲突 (transitively conflict)，Sk <p Sk'（即，在核心 K 的程序顺序中，Sk 比 Sk' 早）和 Sk'与 Sj 的传递性冲突。
如果两个数据操作 Di 和 Dj 发生冲突并且它们出现在全局内存顺序中，则两个数据操作会竞争 (race)，而没有由相同核心（线程）i 和 j 进行的一对可传递冲突的同步操作。换句话说，当且仅当存在一对传递冲突的同步操作 Si 和 Sj 使得 Di <m Si <m Sj <m Dj 时，一对冲突的数据操作 Di <m Dj 不是数据竞争。
如果没有数据操作竞争，则 SC 执行是无数据竞争 (DRF) 的。
如果一个程序的所有SC 执行都是DRF，那么它就是DRF。
如果所有 DRF 程序的所有执行都是 SC 执行，则内存一致性模型支持 "SC for DRF programs"。这种支持通常需要一些特殊的操作来进行同步操作。
考虑内存模型 XC。要求程序员或低层次软件确保所有同步操作都在 FENCE 之前和之后，如表 5.8 中所示。

通过围绕同步操作的 FENCE，XC 支持用于 DRF 程序的 SC。虽然证明超出了这项工作的范围，但该结果背后的直觉来自上面讨论的表 5.7 和表 5.8 中的示例。

支持 DRF 程序的 SC 允许许多程序员使用 SC 而不是更复杂的 XC 规则来推理他们的程序，同时受益于 XC 相对于 SC 启用的任何硬件性能改进或简化。问题——不是总是有问题吗？——保证 DRF 的高性能（即，不将太多操作标记为同步）可能具有挑战性。

无法准确确定哪些内存操作可以竞争，因此必须标记为同步。图 5.4 描述了一个执行，其中核心 C2 的存储应该被标记为同步——这决定了 FENCE 是否实际上是必要的——只有当我们可以确定 C1 的初始代码块是否没有停止时，这当然是不可判定的。当不确定是否需要 FENCE 时，可以通过添加 FENCE 来避免不确定性。这总是正确的，但可能会损害性能。在极限情况下，可以用 FENCE 包围所有内存操作，以确保任何程序的 SC 行为。
最后，程序可能存在由于错误而违反 DRF 的数据竞争。坏消息是，在数据竞争之后，执行可能不再服从 SC，迫使程序员推理底层的宽松内存模型（例如 XC）。好消息是，至少在第一次数据竞争之前，所有执行都将服从 SC，仅允许使用 SC 推理进行一些调试 [5]。
![image](https://github.com/kaitoukito/A-Primer-on-Memory-Consistency-and-Cache-Coherence/assets/34410167/84adb676-f8dd-4810-9e0e-32a9c8fa637d)
总而言之，使用宽松内存系统的程序员可以通过两种选择来推理他们的程序：

他们可以直接使用规则来推理模型做什么和不排序（例如，表 5.5 等）或
他们可以插入足够的同步以确保没有数据争用——仍然允许同步争用——并使用相对简单的连续一致性模型来推理他们的程序，这种模型似乎永远不会出现程序顺序之外的内存操作。
我们几乎总是推荐后一种“无数据竞争的连续一致性”方法，将前一种方法留给编写同步库或设备驱动程序等代码的专家。
